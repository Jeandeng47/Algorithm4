...
public class P_3_1_14 {
    static final int PUTS = 1_000_000;
    static final int GETS = 1_000;
    static final int DOMAIN = 50_000; // larger key set

    public static void build(boolean[] ops, int[] keys) {
        int n = ops.length;
        for (int i = 0; i < GETS; i++) ops[i] = true;
        for (int i = GETS; i < n; i++) ops[i] = false;
...
// Workload: 1,000,000 puts, 1,000 gets (randomly intermixed), domain=50000
// SequentialSearchST time: 1385.55 ms
// BinarySearchST     time: 58.43 ms

// With GETS = 10^3, PUTS = 10^6
// BinarySearchST wins because puts are O(log N) vs O(N) for SequentialSearchST